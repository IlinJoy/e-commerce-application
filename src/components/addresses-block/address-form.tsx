...
export function AddressBlockForm({ address, type, isDefault, isNew = false, onRemove }: AddressFormProps) {
  ...
  const checkboxName = type === 'ShippingAddress' ? 'shippingDefaultAddress' : 'billingDefaultAddress';

  return (
    <form onSubmit={handleSubmit((data) => console.log(data))}>
      <div>
        <CheckBox name={checkboxName} control={control} label="Default Address" disabled={shouldDisabled}></CheckBox>
        <div>
          {!isNew && (
          ...
          <IconButton onClick={() => onRemove(type, address?.id)}>
            <DeleteIcon />
          </IconButton>
        </div>
      </div>
